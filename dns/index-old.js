...
const queryServer = (dnsServer, useTCP, ...questions) => new Promise((resolve, reject) => {
    ...
    if(useTCP) {
        ...
        const sock = net.createConnection({host: dnsServer, port: 53}, () => {
            ...
            sock.write(lengthHeader);
            sock.write(message);
            sock.on("drain", () => sock.end());
            ...
            sock.on("data", (data) => {
                ...
                // grab length prefix when ready
                if(!length && buf.length > 2) {
                    const header = buf.slice(0, 2);
                    buf = buf.slice(2, buf.length);
                    length = header.readUInt16BE();
                }

                // if enough data is received, decode
                if(buf.length >= length) {
                    const reader = new DNSProtocol.DNSReader(buf);
                    resolve(DNSProtocol.DNSMessage.read(reader));
                }

            });

            // kill the socket after long enough
            setTimeout(() => {
